...
export class AuthService {
  ...
  async login(
    ...
    // Find user by email
    const user = await this.userService.findUserByEmailForAuth(loginDto.email);
    ...
    const transformedUser = this.userService.transformUserResponse(user);

    return { message: 'Login successful', token, user: transformedUser };
  }
